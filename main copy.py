...
class MainWindow(tk.CTk):
  ...
  def __init__(self):
    ...
    tk.CTkLabel(self, text="Welcome to Waveify!", font=("Gotham-Bold", 32)).pack(side="top",padx=20,pady=20)
    ...
    self.cover_frame = tk.CTkFrame(self.cover_canvas, fg_color="transparent")
  ...
  def get_all_covers(self): # this gets all covers so they can be animated
    list_of_covers = []
    if self.likedSongs:
      # get covers of liked songs
      for key, value in self.likedSongs.items():
        list_of_covers.append(self.getSongArtFromID(value['id']))
      return list_of_covers
      # get covers and loop through them
    else:
      # get covers of spotify top 50
      top_50_playlist_id = "37i9dQZF1DXcBWIGoYBM5M"
      results = sp.playlist_tracks(top_50_playlist_id)
      for track in results['items']:
        list_of_covers.append(track['track']['album']['images'][0]['url'])
      return list_of_covers
  ...
  def left_to_right(self, list_of_images):
    ...
    for i, cover in enumerate(list_of_images):
      ...
      image_lbl.grid(row=0, column=i, padx=15, pady=15)
...
# gets playlist names
  def getPlaylists(self):
    ...
    self.cover_canvas.place_forget()
...
# displays playlists
  def displayPlaylists(self):
    ...
    frame.place(relx =0.05, rely=0.17, anchor="nw")
    for x in range(len(playlistNames[0])):
      ...
      if(self.listPlaylistBtns[x].winfo_width() > width):
        ...
        frame.configure(width=350, height=325)
    self.playlistSubmitBtn = tk.CTkButton(self, text="Submit", height=30, command=self.when_submit_btn_clicked).place(relx= 0.05, rely = 0.9, anchor="w")
